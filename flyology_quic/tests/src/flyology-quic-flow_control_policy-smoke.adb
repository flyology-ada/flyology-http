...
begin
   ...
   Reserve_Send (Item, 2, Offset => 0, Length => 4, Status => Status);
   ...
   Reserve_Send (Item, 3, Offset => 0, Length => 1, Status => Status);
   pragma Assert (Status = Stream_Not_Sendable);
   Reserve_Send
     (Item, 4, Offset => Value_Type'Last, Length => 1, Status => Status);
