...
private package Flyology.QUIC.Application_Connection is
   ...
   type Build_Result is record
      ...
      Packet_Length : Natural range 0 .. One_RTT_Sender.Max_Packet_Length := 0;
      Header_Length : Natural range 0 .. One_RTT_Sender.Max_Packet_Length := 0;
   ...
   type Process_Result is record
      ...
      Packet : One_RTT_Receiver.Receive_Result;
   end record;
   ...
   with
     Pre =>
       Is_Initialized (Item)
       and then Plaintext'Length <= One_RTT_Sender.Max_Packet_Length,
     Post =>
   ...
   with
     Pre =>
       Is_Initialized (Item)
       and then Packet'Length <= One_RTT_Sender.Max_Packet_Length
