...
--  Internal transmit path for one QUIC v1 Initial packet.
--
--  The caller supplies plaintext frames, including any required PADDING. The
--  sender encodes the long-header envelope and packet number, applies AES-GCM
--  packet protection, and then applies long-header protection.
private package Flyology.QUIC.Initial_Sender is
   ...
   with
     Pre =>
       ...
       and then Token'Length <= Max_Packet_Length
