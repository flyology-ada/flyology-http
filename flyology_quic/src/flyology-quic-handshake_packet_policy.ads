...
--  Internal, proved QUIC v1 Handshake packet envelope policy.
--
--  Offsets are zero-based from the first element of the supplied datagram.
--  The parser locates the Length field, protected packet number, and end of
--  one Handshake packet without interpreting protected header bits. A
--  protected length below 20 cannot provide the 16-byte header-protection
--  sample that begins four bytes after the packet-number field.
private package Flyology.QUIC.Handshake_Packet_Policy
  with Preelaborate,
       SPARK_Mode => On
is
   ...
   with
     ...
     Post =>
       (if Parse'Result.Status = Parsed then
           ...
           and then Parse'Result.Header.Kind = Long_Header_Policy.Handshake
           and then
