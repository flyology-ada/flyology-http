...
package body Flyology.QUIC.Initial_Sender is
   ...
   is
      Prefix : constant Long_Header_Policy.Encoded_Prefix :=
        Long_Header_Policy.Encode_Protected_V1_Prefix
          (Long_Header_Policy.Initial, Number_Length, Destination, Source);
      Token_Length : constant Varint_Policy.Encoded_Value :=
        Varint_Policy.Encode (Varint_Policy.Value_Type (Token'Length));
      ...
      Number_Offset : constant Natural :=
        Prefix.Length + Token_Length.Length + Natural (Token'Length)
        + Length_Field.Length;
   ...
   begin
      ...
      begin
         ...
         Header
           (Ada.Streams.Stream_Element_Offset (Cursor + 1)
            .. Ada.Streams.Stream_Element_Offset
                 (Cursor + Token_Length.Length)) :=
           Token_Length.Data
             (1 .. Ada.Streams.Stream_Element_Offset (Token_Length.Length));
         Cursor := Cursor + Token_Length.Length;

         if Token'Length > 0 then
            Header
              (Ada.Streams.Stream_Element_Offset (Cursor + 1)
               .. Ada.Streams.Stream_Element_Offset
                    (Cursor + Natural (Token'Length))) := Token;
            Cursor := Cursor + Natural (Token'Length);
         end if;

         Header
           (Ada.Streams.Stream_Element_Offset (Cursor + 1)
            .. Ada.Streams.Stream_Element_Offset
                 (Cursor + Length_Field.Length)) :=
           Length_Field.Data
             (1 .. Ada.Streams.Stream_Element_Offset (Length_Field.Length));
         Cursor := Cursor + Length_Field.Length;
         ...
         Header
           (Ada.Streams.Stream_Element_Offset (Cursor + 1)
            .. Ada.Streams.Stream_Element_Offset
                 (Cursor + Natural (Number_Length))) := Encoded_Number;
...
end Flyology.QUIC.Initial_Sender;
